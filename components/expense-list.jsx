...
const ExpenseList = ({
  ...
  const canDeleteExpenses = (expense) => {
  ...
  const handleDeleteExpense = async (expense) => {
    ...
    try {
      ...
      toast.success("Expense deleted S=successfully");
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {expenses.map((expense) => {
        ...
        const showDeleteOption = canDeleteExpenses(expense);
            ...
            <CardContent className={"py-4"}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {showDeleteOption && (
                    <Button
                      ...
                      size={"icon"}
                      className={
                        "h-8 w-8 rounded-full text-red-500 hover:text-red-700 hover:bg-red-100"
                      }
                      onClick={() => handleDeleteExpense(expense)}
                    >
                      <Trash2 className="h-4 w-4" />
                      <span className="sr-only"> Delete expenses</span>
                    </Button>
                  )}
                </div>
              </div>
              ...
              <div className="mt-3 text-sm flex gap-2 flex-wrap">
                {expense.splits.map((split, idx) => {
                  const splitUser = getUserDetail(split.userId, expense);
